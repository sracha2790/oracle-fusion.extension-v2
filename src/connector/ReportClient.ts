...
export class ReportClient {
  ...
  async getExciseDocument(
    ...
    let exciseReportCsv = await this.runFusionReport(sdk, reportPath, host, path, userid, password, trxId, false);
    let csvDocBuilder = new CsvToDocumentConverter();
    let csvDataJson = csvDocBuilder.parseCSVWithUpperCaseHeaders(exciseReportCsv);
    if (docLevels) {
  ...
  async runFusionReportWithBody(
    ...
    let soapAction = 'http://xmlns.oracle.com/oxp/service/v2/ReportService/runReportRequest';
    return await this.sendGetDocSoapRequestSDK(sdk, host, path, body, soapAction, isDataModel);
  }
  ...
  async runFusionReport(
    ...
    if (parameters && Array.isArray(parameters)) {
      for (let param of parameters) {
        paramsXml += '<v2:item>' + '<v2:name>' + param['name'] + '</v2:name>' + '<v2:values>';
  ...
  async sendGetDocSoapRequestSDK(sdk: AppknitSDK, host, path, reqBody, soapAction, isDataModel): Promise<string> {
    const headers = {
      'User-Agent': 'appknit',
    ...
    try {
      ...
      if (isDataModel) {
        if (
          ...
          let reportBytes;
          reportBytes = resp['Envelope']['Body']['runDataModelResponse']['runDataModelReturn']['reportBytes'];
        ...
        if (
          ...
          let reportBytes;
          reportBytes = resp['Envelope']['Body']['runReportResponse']['runReportReturn']['reportBytes'];
          let buf = Buffer.from(reportBytes, 'base64');
          if (buf[0] === 0xef && buf[1] === 0xbb && buf[2] === 0xbf) {
            buf = buf.slice(3);
          }
