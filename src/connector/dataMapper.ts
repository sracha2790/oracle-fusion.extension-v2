...
export class DataMapper {
  ...
  processIncomingSoapRequest(soapRequest: any): any {
    ...
    if (txHeaders) {
      mappings.headers.push(txHeaders);
    ...
    if (detTxLinesColl) {
      detTaxLineMap = new Map();

      if (detTxLinesColl) {
        const detTxLines = detTxLinesColl["ns:DetailTaxLines"];
        if (detTxLines) {
          if (Array.isArray(detTxLines)) {
            for (const dtLine of detTxLines) {
              if (
                (!dtLine["ns:CancelFlag"] || dtLine["ns:CancelFlag"] == "N")
              ) {
                mappings.detTaxLines.push(dtLine);
                this.addDetailsIfVBT(dtLine, mappings);
                detTaxLineMap.set(dtLine["ns:TrxLineId"], dtLine);
              }
            }
          } else {
            if (
              (!detTxLines["ns:CancelFlag"] ||
                detTxLines["ns:CancelFlag"] == "N")
            ) {
              mappings.detTaxLines.push(detTxLines);
              this.addDetailsIfVBT(detTxLines, mappings);
              detTaxLineMap.set(detTxLines["ns:TrxLineId"], detTxLines);
            }
          }
        }
      }
    ...
    if (txLinesColl) {
      const txLine = txLinesColl["ns:TaxableLine"];
      if (txLine) {
        if (Array.isArray(txLine)) {
          for (const line of txLine) {
            this.extractAddresses(line);
            mappings.lines.push(line);
            if (detTaxLineMap) {
              const dtl = detTaxLineMap.get(line["ns:TrxLineId"]);
              if (dtl) {
                lineDetLine.push({ line, dtl });
              }
            }
          }
        } else {
          this.extractAddresses(txLine);
          mappings.lines.push(txLine);
          if (detTaxLineMap) {
            const dtl = detTaxLineMap.get(txLine["ns:TrxLineId"]);
            if (dtl) {
              lineDetLine.push({ txLine, dtl });
            }
          }
        }
      }
    ...
    const mappedRequest = {};
    mappings.linesDetails = lineDetLine;
    mappings.wsAction = action;
    mappedRequest["mappings"] = mappings;
    mappedRequest["request"] = soapRequest;
  ...
  extractAddresses(taxableLine): void {
    ...
    const finalDischarge = this.extractAddressForType(
      taxableLine,
      "ns:FinalDischarge"
    );
