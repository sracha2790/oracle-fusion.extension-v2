...
export class CsvToDocumentConverter {
  parseCSVWithUpperCaseHeaders = (csvData: string) => {
    ...
    const upperCaseHeaders = headers[0].map((header: string) =>
      header.toUpperCase()
    );
  ...
  createLine(line, row) {
    ...
    line["ns:FinalDischargeGeographyType1"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE1"];
    line["ns:FinalDischargeGeographyType10"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE10"];
    line["ns:FinalDischargeGeographyType2"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE2"];
    line["ns:FinalDischargeGeographyType3"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE3"];
    line["ns:FinalDischargeGeographyType4"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE4"];
    line["ns:FinalDischargeGeographyType5"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE5"];
    line["ns:FinalDischargeGeographyType6"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE6"];
    line["ns:FinalDischargeGeographyType7"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE7"];
    line["ns:FinalDischargeGeographyType8"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE8"];
    line["ns:FinalDischargeGeographyType9"] =
      row["FINAL_DISCHARGE_GEOGRAPHY_TYPE9"];
    line["ns:TaxProcessingCompletedFlag"] =
      row["TAX_PROCESSING_COMPLETED_FLAG"];
  ...
  translateDocLevelDefinition(levelDef: DocLevelDefinition) {
    if (levelDef) {
      if (levelDef.levelKeysStr) {
        levelDef.levelKeys = levelDef.levelKeysStr
          .replace(/\r?\n|\r/g, "")
          .split(",");
      ...
      if (
        levelDef.docKeysStr &&
        levelDef.docKeysStr != "*" &&
        !levelDef.docKeysStr.startsWith("*_")
      ) {
        levelDef.docKeys = levelDef.docKeysStr
          .replace(/\r?\n|\r/g, "")
          .split(",");
      ...
      if (levelDef.levelFieldsStr && levelDef.levelFieldsStr != "*") {
        levelDef.levelFields = levelDef.levelFieldsStr
          .replace(/\r?\n|\r/g, "")
          .split(",");
      ...
      if (
        levelDef.docFieldsStr &&
        levelDef.docFieldsStr != "*" &&
        !levelDef.docFieldsStr.startsWith("*_")
      ) {
        levelDef.docFields = levelDef.docFieldsStr
          .replace(/\r?\n|\r/g, "")
          .split(",");
  ...
  traverseDocLevel(
    parentItem: any,
    row: any,
    childLevelStructure: DocLevelDefinition
  ) {
    ...
    if (children && !childLevelStructure.isArray) {
      ...
      if (childLevelStructure.isArray) {
        ...
        if (keysSize > 0) {
          let docKeys = childLevelStructure.docKeys
            ? childLevelStructure.docKeys
            : childLevelStructure.levelKeys;
